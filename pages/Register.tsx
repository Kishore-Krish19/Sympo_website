...
const Register: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (type === "ev") {
      ...
      const workshopId = type.replace("workshop", ""); // "1" | "2" | "3"

      const workshopPrices: Record<string, number> = {
        "1": 300,
        "2": 200,
        "3": 200,
      };

      calculatedAmount = workshopPrices[workshopId] ?? 300;
    } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    const eventType =
      type === "tech" || type === "workshop" || type === "ev" || type === "shark-tank"
        ? "Technical"
        : "Non-Technical";
          ...
          <div className="border border-[var(--border-color)] rounded-xl p-6 bg-[var(--bg-surface)] space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm font-body">
              ...
              <div className="col-span-2 border-t border-[var(--border-color)] pt-2">
                ...
                <span className="text-[var(--accent-orange)] font-bold">{BANK_DETAILS.upiId}</span>
              </div>
              ...
              <div className="col-span-2 flex flex-col items-center justify-center p-4 bg-white rounded-lg border border-[var(--border-color)] mt-4">
                ...
                <p className="text-[var(--accent-blue)] text-sm font-bold mt-4 font-mech tracking-wide">
                  Scan to Pay <span className="text-black text-lg">₹{formData.amount}</span>
                </p>
                <p className="text-xs text-black mt-1">Exact amount pre-filled</p>
              </div>
            </div>
